...
# Админы (добавь свои ID)
ADMINS = [1009623720, 6296059302]  # ← твои ID
...
def moderate(message):
    ...
    if user_id in ADMINS:
        return  # Админы полностью игнорируются
    ...
    if warnings[user_id] >= 2:
        ...
        try:
            bot.delete_message(message.chat.id, message.message_id)
            bot.send_message(message.chat.id, f"⚠️ Предупреждение {warnings[user_id]}/2 — {username}")
        except:
            pass

print("✅ Улучшенная версия запущена!")
